...
export default {
  ...
  computed: {
    ...
    redirectPath () {
      // ログイン済みユーザー｜「homePath（index、RACKのトップページ）」へリダイレクト
      const loggedInHomePath = this.$store.state.loggedIn.homePath
      // ログイン前のユーザー｜「index（RACKのトップページ）」へリダイレクト
  ...
  methods: {
    // リダイレクトパスが現在のルートと一致している場合はリロードを行なう
    // 例）トップページからトップページへリダイレクトするような場面（なんかダメだったことを伝えるためにリロードだけはしてあげるイメージ？）
    redirect () {
