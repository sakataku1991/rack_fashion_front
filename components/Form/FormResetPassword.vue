  ...
  <div class="form-reset-password-container">
    <header class="form-header">
      <h2 class="form-header-title">
        <icon-base
          class="form-header-title-icon icon icon-password"
          height="24"
          icon-name="password"
          width="24"
        >
        ...
        <span class="form-header-title-text">パスワードの再設定</span>
      </h2>
      <div class="form-header-description text-box">
        <p class="form-header-description-text text">
          会員登録時に送信されたメールアドレスを入力してください。パスワード再設定ページへのメールをお送りします。
        </p>
      </div>
    </header>
    <form
      ...
      action="/login"
      method="post"
      class="form form-resetPassword -authentication"
    >
      ...
      <div class="form-section -submit">
        <div class="form-btn -formResetPassword">
          <button
            type="submit"
            value="送信する"
            ontouchstart=""
            class="btn -normal -blue btn-resetPassword btn-text"
          >
            送信する
          </button>
        </div>
...
<script>
export default {
  name: 'ComponentsFormResetPassword'
}
</script>
...
// フォームの内容
.form-header + .form-resetPassword {
  @include sp {
    margin-top: 32px;
  };
  @include pc {
